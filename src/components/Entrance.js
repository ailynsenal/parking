...
export const Entrance = ({ entranceNumber, index }) => {
  ...
  const getNearestParkingSlot = (entranceNumber) => {
    ...
    let availableSlotList =
      entranceNumber > 3 ? availableSlots.reverse() : availableSlots;
    let slotToPark = availableSlotList.map((slot) => {
      let distance = slot.location.split("");
      // calculated based on cartersian plane formula
      slot.distanceLocation = Math.sqrt(
        Math.pow(getDistance(distance[0], entrancePoints[0]), 2) +
          Math.pow(getDistance(distance[1], entrancePoints[1]), 2),
      );
    ...
    <>
      <div className="entrance">
        <div className="entrance-content">
          ...
          <select id="car-sizes" value={size} onChange={onChangeSize}>
        ...
        {isDuplicateEntry && <p className="alert">Car is already parked!</p>}
        {isNullPlateNumber && (
          <p className="alert">Please input or generate plate number!</p>
        )}
        {isFullParking && (
          <p className="alert">No available slot for chosen car size!</p>
        )}
      </div>
    </>
  );
